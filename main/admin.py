...
from .models import City, Taluka, District, State, Pradesh, BhagatDetail, Karyakarta, Zone
...
class TalukaAdmin(ExportActionMixin, admin.ModelAdmin):
    ...
    search_fields = ['talukaName',
                     'district__districtName', 'state__stateName']


class CityAdmin(ExportActionMixin, admin.ModelAdmin):
    list_display = ('cityName', 'pradesh', 'taluka',
                    'district', 'state', 'pincode')
    search_fields = ['cityName', 'pradesh__pradeshName', 'taluka__talukaName',
                     'district__districtName', 'state__stateName', 'pincode']
...
class BhagatDetailAdmin(ExportActionMixin, admin.ModelAdmin):
    list_display = ("name", "districtCode", "receiptNo", "entryDate", "endDate", "subYear", "addressOne",
                    "addressTwo", "addressThree", "city", "taluka", "district", "state", 'pradesh', "phoneNum","pincode","dataid")
    search_fields = ["name", "districtCode", "receiptNo", "entryDate", "endDate", "subYear", "addressOne", "addressTwo", "addressThree",
                     "city__cityName", "taluka__talukaName", "district__districtName", "state__stateName", 'pradesh__pradeshName', "phoneNum", "dataid"]
...
class KaryakartaAdmin(ExportActionMixin, admin.ModelAdmin):
    list_display = ("name", "email", "phoneNum", "city",
                    "taluka", "district", "state")
    search_fields = ["name", "email", "phoneNum", "city__cityName",
                     "taluka__talukaName", "district__districtName", "state__stateName"]
    list_editable = ("email", "phoneNum", "city",
                     "taluka", "district", "state")
